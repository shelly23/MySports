
	 
	/*
	 *	This content is generated from the API File Info.
	 *	(Alt+Shift+Ctrl+I).
	 *
	 *	@desc 		
	 *	@file 		log_in_page
	 *	@date 		Saturday 01st of October 2022 02:28:35 PM
	 *	@title 		Page 1
	 *	@author 	
	 *	@keywords 	
	 *	@generator 	Export Kit v1.3.figma
	 *
	 */
	

package com.example.mysports.activities;

import android.app.Activity;
import android.os.Bundle;


import android.view.View;
import android.widget.ImageView;
import android.widget.TextView;

import com.example.mysports.R;

    public class statistik_activity extends Activity {

	
	private View _bg__statistik_ek2;
	private ImageView background_image_ek9;
	private ImageView path_ek24;
	private ImageView path_ek25;
	private ImageView path_ek26;
	private ImageView oval_1_ek8;
	private ImageView oval_1_copy_ek8;
	private ImageView oval_1_copy_2_ek8;
	private ImageView shape_ek24;
	private ImageView shape_ek25;
	private TextView figma_ek8;
	private ImageView shape_ek26;
	private ImageView charge_ek8;
	private ImageView ___ek8;
	private TextView _42__ek8;
	private ImageView vector_3_ek8;
	private TextView _9_42_am_ek8;
	private View menu_bar_ek15;
	private ImageView vector_ek146;
	private ImageView vector_ek147;
	private ImageView vector_ek148;
	private ImageView vector_ek149;
	private ImageView vector_ek150;
	private ImageView vector_ek151;
	private ImageView vector_ek152;
	private ImageView vector_ek153;
	private ImageView vector_ek154;
	private ImageView vector_ek155;
	private View popup_ek2;
	private TextView _100_;
	private TextView _50_;
	private TextView _0_;
	private TextView aug;
	private TextView jul;
	private TextView jun;
	private TextView mai;
	private TextView apr;
	private TextView m_r;
	private TextView feb;
	private TextView j_n;
	private View line_10;
	private View line_11;
	private View line_18;
	private View line_17;
	private View line_16;
	private View line_15;
	private View line_14;
	private View line_13;
	private View line_12;
	private TextView verlauf_aktiver_tage;
	private View popup_ek3;
	private View kraftlegende;
	private TextView krafttraining;
	private TextView ausdauertraining;
	private View ausdauer_legende;
	private View rectangle_43;
	private View rectangle_44;
	private View rectangle_41;
	private View rectangle_42;
	private View rectangle_39;
	private View rectangle_40;
	private View rectangle_37;
	private View rectangle_38;
	private View rectangle_35;
	private View rectangle_36;
	private View rectangle_33;
	private View rectangle_34;
	private View rectangle_31;
	private View rectangle_32;
	private TextView _100__ek1;
	private TextView _50__ek1;
	private TextView _0__ek1;
	private TextView aug_ek1;
	private TextView jul_ek1;
	private TextView jun_ek1;
	private TextView mai_ek1;
	private TextView apr_ek1;
	private TextView m_r_ek1;
	private TextView feb_ek1;
	private TextView j_n_ek1;
	private View line_10_ek1;
	private View line_11_ek1;
	private View rectangle_29;
	private View rectangle_30;
	private TextView trainingszusammensetzung;
	private ImageView front_ek2;
	private ImageView front_ek3;
	private ImageView back_ek2;
	private ImageView back_ek3;

	@Override
	public void onCreate(Bundle savedInstanceState) {

		super.onCreate(savedInstanceState);
		setContentView(R.layout.statistik);

		
		_bg__statistik_ek2 = (View) findViewById(R.id._bg__statistik_ek2);
		background_image_ek9 = (ImageView) findViewById(R.id.background_image_ek9);
		path_ek24 = (ImageView) findViewById(R.id.path_ek24);
		path_ek25 = (ImageView) findViewById(R.id.path_ek25);
		path_ek26 = (ImageView) findViewById(R.id.path_ek26);
		oval_1_ek8 = (ImageView) findViewById(R.id.oval_1_ek8);
		oval_1_copy_ek8 = (ImageView) findViewById(R.id.oval_1_copy_ek8);
		oval_1_copy_2_ek8 = (ImageView) findViewById(R.id.oval_1_copy_2_ek8);
		shape_ek24 = (ImageView) findViewById(R.id.shape_ek24);
		shape_ek25 = (ImageView) findViewById(R.id.shape_ek25);
		figma_ek8 = (TextView) findViewById(R.id.figma_ek8);
		shape_ek26 = (ImageView) findViewById(R.id.shape_ek26);
		charge_ek8 = (ImageView) findViewById(R.id.charge_ek8);
		___ek8 = (ImageView) findViewById(R.id.___ek8);
		_42__ek8 = (TextView) findViewById(R.id._42__ek8);
		vector_3_ek8 = (ImageView) findViewById(R.id.vector_3_ek8);
		_9_42_am_ek8 = (TextView) findViewById(R.id._9_42_am_ek8);
		menu_bar_ek15 = (View) findViewById(R.id.menu_bar_ek15);
		vector_ek146 = (ImageView) findViewById(R.id.vector_ek146);
		vector_ek147 = (ImageView) findViewById(R.id.vector_ek147);
		vector_ek148 = (ImageView) findViewById(R.id.vector_ek148);
		vector_ek149 = (ImageView) findViewById(R.id.vector_ek149);
		vector_ek150 = (ImageView) findViewById(R.id.vector_ek150);
		vector_ek151 = (ImageView) findViewById(R.id.vector_ek151);
		vector_ek152 = (ImageView) findViewById(R.id.vector_ek152);
		vector_ek153 = (ImageView) findViewById(R.id.vector_ek153);
		vector_ek154 = (ImageView) findViewById(R.id.vector_ek154);
		vector_ek155 = (ImageView) findViewById(R.id.vector_ek155);
		popup_ek2 = (View) findViewById(R.id.popup_ek2);
		_100_ = (TextView) findViewById(R.id._100_);
		_50_ = (TextView) findViewById(R.id._50_);
		_0_ = (TextView) findViewById(R.id._0_);
		aug = (TextView) findViewById(R.id.aug);
		jul = (TextView) findViewById(R.id.jul);
		jun = (TextView) findViewById(R.id.jun);
		mai = (TextView) findViewById(R.id.mai);
		apr = (TextView) findViewById(R.id.apr);
		m_r = (TextView) findViewById(R.id.m_r);
		feb = (TextView) findViewById(R.id.feb);
		j_n = (TextView) findViewById(R.id.j_n);
		line_10 = (View) findViewById(R.id.line_10);
		line_11 = (View) findViewById(R.id.line_11);
		line_18 = (View) findViewById(R.id.line_18);
		line_17 = (View) findViewById(R.id.line_17);
		line_16 = (View) findViewById(R.id.line_16);
		line_15 = (View) findViewById(R.id.line_15);
		line_14 = (View) findViewById(R.id.line_14);
		line_13 = (View) findViewById(R.id.line_13);
		line_12 = (View) findViewById(R.id.line_12);
		verlauf_aktiver_tage = (TextView) findViewById(R.id.verlauf_aktiver_tage);
		popup_ek3 = (View) findViewById(R.id.popup_ek3);
		kraftlegende = (View) findViewById(R.id.kraftlegende);
		krafttraining = (TextView) findViewById(R.id.krafttraining);
		ausdauertraining = (TextView) findViewById(R.id.ausdauertraining);
		ausdauer_legende = (View) findViewById(R.id.ausdauer_legende);
		rectangle_43 = (View) findViewById(R.id.rectangle_43);
		rectangle_44 = (View) findViewById(R.id.rectangle_44);
		rectangle_41 = (View) findViewById(R.id.rectangle_41);
		rectangle_42 = (View) findViewById(R.id.rectangle_42);
		rectangle_39 = (View) findViewById(R.id.rectangle_39);
		rectangle_40 = (View) findViewById(R.id.rectangle_40);
		rectangle_37 = (View) findViewById(R.id.rectangle_37);
		rectangle_38 = (View) findViewById(R.id.rectangle_38);
		rectangle_35 = (View) findViewById(R.id.rectangle_35);
		rectangle_36 = (View) findViewById(R.id.rectangle_36);
		rectangle_33 = (View) findViewById(R.id.rectangle_33);
		rectangle_34 = (View) findViewById(R.id.rectangle_34);
		rectangle_31 = (View) findViewById(R.id.rectangle_31);
		rectangle_32 = (View) findViewById(R.id.rectangle_32);
		_100__ek1 = (TextView) findViewById(R.id._100__ek1);
		_50__ek1 = (TextView) findViewById(R.id._50__ek1);
		_0__ek1 = (TextView) findViewById(R.id._0__ek1);
		aug_ek1 = (TextView) findViewById(R.id.aug_ek1);
		jul_ek1 = (TextView) findViewById(R.id.jul_ek1);
		jun_ek1 = (TextView) findViewById(R.id.jun_ek1);
		mai_ek1 = (TextView) findViewById(R.id.mai_ek1);
		apr_ek1 = (TextView) findViewById(R.id.apr_ek1);
		m_r_ek1 = (TextView) findViewById(R.id.m_r_ek1);
		feb_ek1 = (TextView) findViewById(R.id.feb_ek1);
		j_n_ek1 = (TextView) findViewById(R.id.j_n_ek1);
		line_10_ek1 = (View) findViewById(R.id.line_10_ek1);
		line_11_ek1 = (View) findViewById(R.id.line_11_ek1);
		rectangle_29 = (View) findViewById(R.id.rectangle_29);
		rectangle_30 = (View) findViewById(R.id.rectangle_30);
		trainingszusammensetzung = (TextView) findViewById(R.id.trainingszusammensetzung);
		front_ek2 = (ImageView) findViewById(R.id.front_ek2);
		front_ek3 = (ImageView) findViewById(R.id.front_ek3);
		back_ek2 = (ImageView) findViewById(R.id.back_ek2);
		back_ek3 = (ImageView) findViewById(R.id.back_ek3);
	
		
		//custom code goes here
	
	}
}
	
	